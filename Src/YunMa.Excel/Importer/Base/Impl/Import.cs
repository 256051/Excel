...
using YunMa.Excel.Core.Attributes;
...
using YunMa.Excel.Utility;
using LicenseContext = OfficeOpenXml.LicenseContext;


namespace YunMa.Excel.Importer.Base.Impl
{
    /// <summary>
    /// 导入
    /// </summary>
    /// <typeparam name="T"></typeparam>
    internal class Import<T> : IImport<T> where T : class, new()
    {
        ...
        private Dictionary<string, dynamic> _dicMergePreValues = new();
        /// <summary>
        ///     导入文件路径
        /// </summary>
        ...
        ///     标注文件路径
        /// </summary>
        private string LabelingFilePath { get; }

        /// <summary>
        ///     导入结果
        /// </summary>
        private ImportResult<T> ImportResult { get; set; }

        /// <summary>
        ///     列头定义
        /// </summary>
        private List<ImporterHeaderInfo> ImporterHeaderInfos { get; set; }
        ...
        ///     错误文件流
        /// </summary>
        private Stream LabelStream { get; set; }
        ...
        ///     导入全局设置
        /// </summary>
        private ExcelImporterAttribute ExcelImporterSettings
        {
            get
            {
        ...
        /// <summary>
        /// </summary>
        /// <param name="importResultFilters"></param>
        /// <param name="stream"></param>
        /// <param name="importHeaderFilters"></param>
        public Import(IEnumerable<IImportHeaderFilter> importHeaderFilters, IEnumerable<IImportResultFilter> importResultFilters, Stream stream)
        {
        ...
        /// <param name="importHeaderFilters"></param>
        public Import(IEnumerable<IImportHeaderFilter> importHeaderFilters, IEnumerable<IImportResultFilter> importResultFilters, Stream stream, Stream labelStream) : this(importHeaderFilters, importResultFilters, stream)
        {
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            Resource.Culture = new CultureInfo("zh-CN");
        ...
        /// <param name="importHeaderFilters"></param>
        public Import(IEnumerable<IImportHeaderFilter> importHeaderFilters, IEnumerable<IImportResultFilter> importResultFilters, string filePath = null, string labelingFilePath = null)
        {
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            Resource.Culture = new CultureInfo("zh-CN");
            _importHeaderFilters = importHeaderFilters;
        ...
        /// <param name="labelStream"></param>
        /// <param name="importHeaderFilters"></param>
        public Import(IEnumerable<IImportHeaderFilter> importHeaderFilters,
            IEnumerable<IImportResultFilter> importResultFilters, string filePath, Stream labelStream)
        {
            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;
            Resource.Culture = new CultureInfo("zh-CN");
        ...
        private void Dispose(bool disposing)
        {
            ...
            if (disposing)
            {
                ...
                if (_dicMergePreValues != null)
                {
                    _dicMergePreValues = null;
                }
            }
        ...
        /// </summary>
        /// <returns></returns>
        public Task<ImportResult<T>> ImportExcel(string filePath = null)
        {
            ...
            try
            {
                ...
                using (Stream)
                {
                    ...
                    ParseData(excelPackage, out var hasCloseNote);
                    if (!string.IsNullOrWhiteSpace(ExcelImporterSettings.ClosingNote) && !hasCloseNote)
                    {
                        throw new ArgumentException($"Excel文档格式错误，未包含结束符：{ExcelImporterSettings.ClosingNote}");
                    }
                    #region 数据验证
        ...
        {
            //ImporterHeaderInfos
            if (ImporterHeaderInfos.Count == 0) return;
            ...
            foreach (var colHeaderInfo in ImporterHeaderInfos)
            {
                //查询指定列
                var qDataByProp = qDataList.Select($"new(RowData.{colHeaderInfo.PropertyName} as Value, RowIndex)")
                    .OrderBy("RowIndex").ToDynamicList();
                for (var i = 0; i < qDataByProp.Count; i++)
                {
                    //上一行的值
                    var preValue = qDataByProp[i].Value;
                    if (string.IsNullOrEmpty(preValue?.ToString())) continue;


                    var header = colHeaderInfo.Header;

                    var key = header.Name ??
                              colHeaderInfo.PropertyName;

                    if (header.IsAllowChsChar)
                    {
                        var cur = preValue?.ToString();
                        if (cur != null && !ValidatorHelper.ValidChineseInput(cur))
                        {
                            var dataRowError = GetDataRowErrorInfo(qDataByProp[i].RowIndex);
                            if (dataRowError.FieldErrors.ContainsKey(key))
                                dataRowError.FieldErrors[key] += Environment.NewLine + $"{Resource.AllowChsEngChar}";
                            else
                                dataRowError.FieldErrors.Add(key, $"{Resource.AllowChsEngChar}");
                        }
                    }

                    if (header.IsAllowEngChar)
                    {
                        var cur = preValue?.ToString();
                        if (cur != null && !ValidatorHelper.ValidEnglishInput(cur))
                        {
                            var dataRowError = GetDataRowErrorInfo(qDataByProp[i].RowIndex);
                            if (dataRowError.FieldErrors.ContainsKey(key))
                                dataRowError.FieldErrors[key] += Environment.NewLine + $"{Resource.AllowEngChar}";
                            else
                                dataRowError.FieldErrors.Add(key, $"{Resource.AllowEngChar}");
                        }
                    }
                }
            }
        ...
        private void RepeatDataCheck()
        {
            ...
            foreach (var notAllowRepeatCol in notAllowRepeatCols)
            {
                ...
                for (var i = 0; i < qDataByProp.Count; i++)
                {
                    ...
                    if (notAllowRepeatCol.Header.IgnoreUpperLower)
                    {
                        var cur = currentValue?.ToString()?.ToLower();
                        var pre = preValue?.ToString()?.ToLower();
                        if (cur == pre)
                        {
                    ...
                    var errorIndexStr = string.Join("，", listRepeatRows.Distinct());
                    foreach (var repeatRow in listRepeatRows.Distinct())
                    {
                        ...
                        if (!notAllowRepeatCol.Header.IsAllowRepeat)
                        {
                            error = $"{Resource.ExistDuplicateData}{errorIndexStr}";
                        }

                        if (notAllowRepeatCol.Header.IgnoreUpperLower)
                        {
                            error = $"{Resource.ExistDuplicateDataNoFixUpper}{errorIndexStr}";
                        }
        ...
        private void LabelingError(ExcelPackage excelPackage)
        {
            //如果源路径为空则不允许生成标注文件
            if (string.IsNullOrWhiteSpace(FilePath) && LabelStream == null)
            {
                return;
            }
            ...
            if (ExcelImporterSettings.IsLabelingError && ImportResult.HasError)
            {
                ...
                foreach (var item in ImportResult.RowErrors)
                {
                    ...
                    foreach (var field in item.FieldErrors)
                    {
                        ...
                        // if (col == null)
                        // {
                        //     throw new ArgumentException($"表头列名{field.Key}不存在");
                        // }
                ...
                if (!string.IsNullOrWhiteSpace(LabelingFilePath))
                {
                    excelPackage.SaveAs(new FileInfo(LabelingFilePath));
                }
                // var filePath = string.IsNullOrWhiteSpace(LabelingFilePath)
                //     ? FilePath.Replace(ext, "_" + ext)
                //     : LabelingFilePath;
                // excelPackage.SaveAs(new FileInfo(filePath));
                if (LabelStream == null) return;
                excelPackage.SaveAs(LabelStream);
        ...
        private void LabelingBussinessError(ExcelPackage excelPackage,
        ...
        /// </summary>
        /// <returns></returns>
        private void ParseTemplate(ExcelPackage excelPackage)
        {
            ...
            try
            {
                ...
                if (worksheet == null)
                {
                    throw new ArgumentException($"Sheet名称错误，请将Sheet命名为：{ExcelImporterSettings.SheetName}");
                }
        ...
        private bool ParseHeader()
        {
            ...
            foreach (var propertyInfo in objProperties)
            {
                ...
                var ignore = (propertyInfo.GetAttribute<IgnoreAttribute>(true) == null)
                    ? importerHeaderAttribute.IsIgnore
                    : propertyInfo.GetAttribute<IgnoreAttribute>(true).IsImportIgnore;
                //忽略字段处理
                if (ignore) continue;
        ...
        // {
        //     var filters = filterType.GetFilter<TFilter>(ExcelImporterSettings.IsDisableAllFilter);
        //     return filterType != null ? filters.FirstOrDefault(x => x.GetType() == filterType) : filters.FirstOrDefault();
        // }
        ...
        private TFilter GetFilter<TFilter>(Type filterType = null) where TFilter : IFilter
        {
            if (ExcelImporterSettings.IsDisableAllFilter)
            {
                return default;
            }
            ...
            var isImportHeaderFilter = filterType.GetInterfaces().Any(i => i == typeof(IImportHeaderFilter));
            if (isImportHeaderFilter)
            {
                var filter = _importHeaderFilters.FirstOrDefault(x => x.GetType() == filterType);
                return (TFilter)filter;
            }
            var isImportResultFilter= filterType.GetInterfaces().Any(i => i == typeof(IImportResultFilter));
            if (isImportResultFilter)
            {
                var filter = _importResultFilters.FirstOrDefault(x => x.GetType() == filterType);
                return (TFilter)filter;
            }
            return default;
        }
        ...
        ///     构建Excel模板
        /// </summary>
        private void StructureExcel(ExcelPackage excelPackage)
        {
            var worksheet =
            ...
            for (var i = 0; i < ImporterHeaderInfos.Count; i++)
            {
                ...
                if (ImporterHeaderInfos[i].MappingValues.Count > 0)
                {
                    ...
                    var hiddenWorksheet =
                        excelPackage.Workbook.Worksheets.Add($"hidden_{ImporterHeaderInfos[i].PropertyName}");
                    ...
                    dataValidations.Formula.ExcelFormula = $"hidden_{ImporterHeaderInfos[i].PropertyName}!$A$1:$A$" +
                                                           ImporterHeaderInfos[i].MappingValues.Count;
                }
                ...
                if (ImporterHeaderInfos[i].Header.IsInterValidation)
                {
                    ...
                    SetInterValidation(worksheet, ImporterHeaderInfos[i].PropertyInfo, range,
                        ImporterHeaderInfos[i].Header.ShowInputMessage);
                }
        ...
        private void ParseData(ExcelPackage excelPackage, out bool hasCloseNote)
        {
            var worksheet = GetImportSheet(excelPackage);
            hasCloseNote = false;
            //检查导入最大条数限制
                ...
                rowIndex++)
            {
                ...
                if (!string.IsNullOrWhiteSpace(ExcelImporterSettings.ClosingNote) &&
                    worksheet.Cells[rowIndex, 1].Text == ExcelImporterSettings.ClosingNote)
                {
                    hasCloseNote = true;
                    break;
                }
                        ...
                        ImporterHeaderInfos.Any(p1 => p1.PropertyName == p.Name && p1.IsExist)))
                    {
                        ...
                        try
                        {
                            ...
                            if (cell.Merge && cell.Value == null)
                            {
                                ...
                                if (_dicMergePreValues.ContainsKey(key))
                                {
                                    propertyInfo.SetValue(dataItem, _dicMergePreValues[key]);
                                    continue;
                                }
                            }
                            ...
                            switch (propertyInfo.PropertyType.GetCSharpTypeName())
                            {
                                ...
                                case "String":
                                    ...
                                    if (col.Header.FixAllSpace)
                                    {
                                        var value = cellValue;
                                        if (value != null)
                                            value = Regex.Replace(cellValue, @"\s", string.Empty);
                                        SetValue(cell, dataItem, propertyInfo, value);
                                    }
                                    else if (col.Header.FixAllEnter)
                                    {
                                ...
                                case "Int64":
                                    {
                                        if (!long.TryParse(cellValue, out var number) && col.Header.DefaultValue == null)
                                        {
                                            AddRowDataError(rowIndex, col,
                                                $"{Resource.Value} {cellValue} {Resource.PleaseFillInTheCorrectIntegerValue}");
                                            break;
                                        }

                                        if (col.Header.DefaultValue != null)
                                        {
                                            if (!long.TryParse(col.Header.DefaultValue.ToString(), out number))
                                            {
                                                AddRowDataError(rowIndex, col,
                                                    $"{Resource.Value} {cellValue} {Resource.PleaseFillInTheCorrectIntegerValue}");
                                                break;
                                ...
                                case "Int32":
                                    {
                                        // int.TryParse(col.Header.DefaultValue,out var intValue)

                                        if (!int.TryParse(cellValue, out var number) && col.Header.DefaultValue == null)
                                        {
                                            AddRowDataError(rowIndex, col,
                                                $"{Resource.Value} {cellValue} {Resource.PleaseFillInTheCorrectIntegerValue}");
                                            break;
                                        ...
                                        if (col.Header.DefaultValue != null && string.IsNullOrWhiteSpace(cellValue))
                                        {
                                            if (!int.TryParse(col.Header.DefaultValue.ToString(), out number))
                                            {
                                                AddRowDataError(rowIndex, col,
                                                    $"{Resource.Value} {cellValue} {Resource.PleaseFillInTheCorrectIntegerValue}");
                                                break;
        ...
        /// <param name="excelPackage"></param>
        /// <returns></returns>
        private ExcelWorksheet GetImportSheet(ExcelPackage excelPackage)
        {

            var data = excelPackage.Workbook.Worksheets[typeof(T).GetDisplayName()] ??
                       excelPackage.Workbook.Worksheets[ExcelImporterSettings.SheetName];
            return data ?? excelPackage.Workbook.Worksheets[ExcelImporterSettings.SheetIndex];
        }
        ...
        private void AddRowDataError(int rowIndex, ImporterHeaderInfo importerHeaderInfo,
