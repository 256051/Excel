...
{ 
    ...
    ///     多Sheet导入辅助类
    /// </summary>
    internal class ImportMultipleSheet : IImportMultipleSheet
    {
        ...
        private readonly List<PropertyInfo> _sheetPropertyList;

        private ImportMultipleSheet()
        {
        }
        ...
        public ImportMultipleSheet(Stream stream)
        {
            _excelStream = stream;
        }
        ...
        public ImportMultipleSheet(string filePath)
        {
            FilePath = filePath;
        ...
        public ImportMultipleSheet(List<PropertyInfo> sheetPropertyList)
        {
        ...
        ///     导入结果
        /// </summary>
        private ImportResult<object> ImportResult { get; set; }
        ...
        private static void CheckImportFile(string filePath)
        {
            if (string.IsNullOrWhiteSpace(filePath))
                throw new ArgumentException(Resource.FilePathCannotBeEmpty, nameof(filePath));
        ...
        /// </summary>
        /// <returns></returns>
        public Task<ImportResult<object>> Import(string sheetName, int sheetIndex, Type importDataType,
        ...
        private void LabelingError(ExcelPackage excelPackage, bool isSaveLabelingError = true)
        {
            //是否标注错误
        ...
        {
            //获取需要检查重复数据的列
            var notAllowRepeatCols = ImporterHeaderInfos
                .Where(p => p.Header.IsAllowRepeat == false).ToList();
        ...
        private void ParseData(ExcelPackage excelPackage)
        {
            ...
            if (ExcelImporterSettings.MaxCount != 0
                ...
                throw new ArgumentException(
                    $"{Resource.MaximumNumberImportsCannotExceeded}{ExcelImporterSettings.MaxCount}");
                        ...
                        try
                        {
                            ...
                            if (propertyInfo.PropertyType.IsEnum)
                            {
                                AddRowDataError(rowIndex, col,
                                    $"{Resource.Value}{cellValue} {Resource.ThereAreNoTemplateDropDownOptions}");
                                continue;
                            }
                            ...
                            switch (propertyInfo.PropertyType.GetCSharpTypeName())
                            {
                                ...
                                case "Nullable<Boolean>":
                                    if (string.IsNullOrWhiteSpace(cellValue))
                                        ...
                                        AddRowDataError(rowIndex, col,
                                            $"{Resource.Value} {cellValue} {Resource.Illegal}");
                                    break;
                                case "String":
                                    //TODO:进一步优化
                                    //移除所有的空格，包括中间的空格
                                ...
                                case "Decimal":
                                {
                                    ...
                                    {
                                        AddRowDataError(rowIndex, col,
                                            $"{Resource.Value} {cellValue} {Resource.PleaseFillInTheCorrectDecimal}");
                                        break;
                                    }
                                ...
                                case "DateTime":
                                {
                                    ...
                                    {
                                        AddRowDataError(rowIndex, col,
                                            $"{Resource.Value} {cellValue} {Resource.PleaseFillInTheCorrectDateAndTimeFormat}");
                                        break;
                                    }
        ...
        public Task<ExportFileInfo> GenerateTemplate(string fileName = null)
        {
            if (string.IsNullOrWhiteSpace(fileName))
                throw new ArgumentException(Resource.FileNameMustBeFilled, fileName);
        ...
        private bool ParseImporterHeader()
        {
        ...
        /// <returns></returns>
        /// <exception cref="ArgumentException">导入实体没有定义ImporterHeader属性</exception>
        private bool ParseImporterHeader(Type sheetType)
        {
            ImporterHeaderInfos = new List<ImporterHeaderInfo>();
            ...
            foreach (var propertyInfo in objProperties)
            {
                ...
                switch (propertyInfo.PropertyType.GetCSharpTypeName())
                {
                    ...
                    case "Nullable<Boolean>":
                    {
                        if (!colHeader.MappingValues.ContainsKey(Resource.Yes))
                            colHeader.MappingValues.Add(Resource.Yes, true);
                        if (!colHeader.MappingValues.ContainsKey(Resource.No))
                            colHeader.MappingValues.Add(Resource.No, false);
                        break;
                    }
                }
        ...
        private void StructureExcel(ExcelPackage excelPackage)
        {
            foreach (var sheetProperty in _sheetPropertyList)
            {
                ...
                var importerAttribute =
                    (sheetProperty.GetCustomAttributes(typeof(ExcelImporterAttribute),
                        true) as ExcelImporterAttribute[])?.FirstOrDefault();
